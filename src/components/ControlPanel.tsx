...
export default function ControlPanel({ settings, onChange, onReset }: Props) {
  ...
  // Clamp a position so the panel stays fully inside the viewport.
  ...
  // Park the panel at the right edge, vertically centred, on first mount.
  useEffect(() => {
    const el = panelRef.current;
    if (!el) return;
  ...
  const onHeaderPointerDown = (event: React.PointerEvent<HTMLDivElement>) => {
    // Let buttons inside the header work without starting a drag.
    ...
    >
      ...
      {!collapsed && (
        <div className="border-t border-white/10">
          ...
          <div className="max-h-[min(68dvh,31rem)] space-y-3 overflow-y-auto px-4 py-4">
            {sliders.map((slider) => {
